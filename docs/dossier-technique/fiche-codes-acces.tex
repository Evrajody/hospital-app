...
\fiche{Système d'exploitation (hôte / VM)}{
  ...
  \textbf{Mail du compte google Backup} & \code{hzmdbk@gmail.com} \\\hline
  \textbf{Mot de passe compte Backup} & \code{KdV0E4F1\&QzXpW} \\\hline

  \textbf{Mot de passe} & \code{appli2026} \\\hline
  \textbf{Hyperviseur} & VMware Workstation (sur le serveur physique de l'hôpital) \\\hline
  \textbf{Accès distant (SSH)} & Port \code{2203} — à ouvrir ponctuellement sur le routeur, puis à refermer après intervention \\
}
...
\fiche{Moteur PostgreSQL 16 — conteneur \texttt{hospital-db}}{
  ...
  \textbf{Mot de passe} & \code{password} \newline {\small\color{greytext}(valeur de \code{DB\_PASSWORD} dans \code{.env.production} — à remplacer par le secret réel du serveur)} \\\hline
...
\fiche{Ports \& réseau Docker}{
  ...
  \textbf{Administration serveur} & Port SSH \code{2203} — à ouvrir ponctuellement sur le routeur, puis refermer. \\\hline
