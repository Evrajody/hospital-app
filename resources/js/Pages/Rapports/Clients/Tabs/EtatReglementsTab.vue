  ...
  <div class="tab-content">
    ...
    <div class="filters-section">
      <el-form :inline="true" class="filters-form">
        ...
        <el-form-item v-if="selectedMode === 'tous_clients' || selectedMode === 'par_client'" label="Date début">
          <el-date-picker v-model="dateDebut" type="date" format="DD/MM/YYYY" value-format="YYYY-MM-DD" placeholder="Date début" />
        </el-form-item>

        <el-form-item v-if="selectedMode === 'tous_clients' || selectedMode === 'par_client'" label="Date fin">
    ...
    <div v-if="fetched" class="results-section">
      ...
      <template v-if="(selectedMode === 'par_client' || selectedMode === 'un_client') && data.length > 0">
        <div v-for="clientData in data" :key="clientData.client_id" class="client-block">
          ...
          <el-table :data="clientData.lignes" border size="small" stripe>
            ...
            <el-table-column prop="date_reglement" label="Date Règlement" width="120" />
            ...
            <el-table-column label="Rejet" width="100" align="right">
      ...
      <template v-if="selectedMode === 'tous_clients' && data.length > 0">
        <el-table :data="data" border size="small" stripe show-summary :summary-method="getSummary">
          ...
          <el-table-column label="Montant Factures" width="140" align="right">
            <template #default="{ row }">{{ formatMontant(row.total_facture) }}</template>
          </el-table-column>
          <el-table-column label="Montant Règlements" width="150" align="right">
            <template #default="{ row }">{{ formatMontant(row.total_paye) }}</template>
          </el-table-column>
          <el-table-column label="Rejet" width="120" align="right">
            <template #default="{ row }">
...
const getSummary = ({ columns, data: tableData }) => {
  ...
  columns.forEach((col, i) => {
    ...
    const key = i === 3 ? 'total_facture' : i === 4 ? 'total_paye' : 'total_rejet';
