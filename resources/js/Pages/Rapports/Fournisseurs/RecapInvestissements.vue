  ...
  <div class="document-container">
    <div class="document-page">
      ...
      <div class="document-title">
        <h1>ÉTAT RÉCAPITULATIF DES INVESTISSEMENTS</h1>
        <div class="subtitle">Immobilisations</div>
      ...
      <div class="section summary-section">
        <div class="summary-grid">
          <div class="summary-box">
            <div class="summary-label">Total Investissements HT</div>
          ...
          <div class="summary-box">
            <div class="summary-label">Nombre d'acquisitions</div>
            <div class="summary-value">{{ factures.length }}</div>
          </div>
        </div>
      </div>

      <!-- Par type d'immobilisation -->
      <div class="section">
        <h2>RÉPARTITION PAR TYPE D'IMMOBILISATION</h2>
        <table class="data-table">
          <thead>
            <tr>
              <th>Type</th>
              <th>Compte</th>
              <th class="montant-col">Nb</th>
          ...
          <tbody>
            <tr v-for="type in typesImmo" :key="type.compte">
              <td><strong>{{ type.libelle }}</strong></td>
              <td>{{ type.compte }}</td>
              <td class="montant-col">{{ type.nb }}</td>
              <td class="montant-col">{{ formatMontant(type.montant_ht) }}</td>
              <td class="montant-col">{{ formatMontant(type.montant_tva) }}</td>
              <td class="montant-col">{{ formatMontant(type.montant_ttc) }}</td>
              <td class="montant-col">{{ ((type.montant_ttc / totaux.ttc) * 100).toFixed(1) }}%</td>
            </tr>
          </tbody>
          <tfoot>
            <tr class="total-row">
...
const typesImmo = computed(() => {
  const types = {};
  props.factures.forEach(f => {
    const key = f.compte_immo || 'Autres';
    if (!types[key]) {
      types[key] = {
        compte: f.compte_immo,
        libelle: f.type_immo,
        nb: 0,
        montant_ht: 0,
        montant_tva: 0,
    ...
    types[key].nb++;
    types[key].montant_ht += f.montant_ht;
    types[key].montant_tva += f.montant_tva;
    types[key].montant_ttc += f.montant_ttc;
  });
