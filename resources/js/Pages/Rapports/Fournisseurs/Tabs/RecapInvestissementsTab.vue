  ...
  <div class="tab-content">
      ...
      <el-form :inline="true" class="filters-form">
        <el-form-item label="Mode">
          <el-radio-group v-model="mode" size="default">
            <el-radio-button value="toutes">Tous les investissements</el-radio-button>
            <el-radio-button value="par_compte">Par compte</el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="mode === 'par_compte'" label="Compte">
          ...
          >
            <el-option
              v-for="c in comptesInvestissements"
              :key="c.id"
    ...
    <div v-if="fetched" class="results-section">
      <div v-if="lignes.length === 0" class="empty-state">
        <el-empty description="Aucun investissement trouvé pour cette période" />
      </div>
      <template v-else>
...
const REPORT_KEY = 'rapports-fournisseurs.recap-investissements';
...
const titre = ref('');
const lignes = ref([]);
const totalInvestissements = ref(0);
const comptesInvestissements = ref([]);

const loadComptes = async () => {
  try {
    const res = await fetch('/rapports/fournisseurs/api/recap-investissements?load_comptes=1');
    const json = await res.json();
    if (json.comptesDisponibles) comptesInvestissements.value = json.comptesDisponibles;
  } catch (e) { /* silencieux */ }
};
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await fetch(`/rapports/fournisseurs/api/recap-investissements?${params}`);
    const json = await res.json();
    titre.value = json.titre || '';
    lignes.value = json.lignes || [];
    totalInvestissements.value = json.totalDepenses || 0;
    if (json.comptesDisponibles) comptesInvestissements.value = json.comptesDisponibles;
...
const getSummary = ({ columns }) => {
  ...
  return columns.map((col, index) => {
    if (isDetail) {
      if (index === 0) return '';
      if (index === 1) return '';
      if (index === 2) return 'TOTAL INVESTISSEMENTS :';
      if (index === 3) return formatMontant(totalInvestissements.value);
    } else {
      if (index === 0) return '';
      if (index === 1) return 'TOTAL INVESTISSEMENTS :';
      if (index === 2) return formatMontant(totalInvestissements.value);
    }
    return '';
  });
};
...
const printReport = () => {
  ...
  const w = window.open(`/rapports/fournisseurs/pdf/recap-investissements?${params}`, '_blank');
